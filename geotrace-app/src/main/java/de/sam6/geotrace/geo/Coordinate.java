...
public class Coordinate
{
   ...
   /**
    * Create Coordinate from a call like {@code new Coordinate(51, "W", 1, "E");}.
    * @param latitude
    * @param dirLa
    * @param longitude
    * @param dirLo
   ...
   /**
    * Is this Coordiante equal to another? Yes if it is on the same Lon & Lat, within +/- eps.
    * @param co
    * @return
    */
   public boolean equals(Coordinate co)
   {
      ...
      if((co.lat == lat && co.lon == lon && co.dirLat == dirLat && co.dirLon == dirLon) ||
         (co.lat >= lat - eps && co.lat <= lat + eps && co.lon >= lon - eps && co.lon <= lon + eps &&
