...
public class Trace extends Thread
{
   ...
   /**
    * Read the output from the traceroute utitlity and add it to the Collection Main.traces
    */
   public void run()
   {
      started = true;
      ...
      {
         String line;
         Process p = Runtime.getRuntime().exec(this.p.configFile.getProperty("TRACEROUTE_PATH") + " " + hostname);
         ...
         while((line = input.readLine()) != null)
         {
            ...
            if(tr != null)
            {
               ...
               if(tr.geolocation != null && this.p.connections.size() >= 1)
               {
                  Connection tr_prev = this.p.connections.get(this.p.connections.size() - 1);
                  if(tr_prev.co2.equals(tr.geolocation) == false)
                  {
                     addNewConnection(this.p.connections.get(this.p.connections.size() - 1).co2, tr.geolocation);
                  }
               }

               if(tr.geolocation != null && this.p.connections.size() == 0)
               {
                  Iterator<TraceResult> tr_ = this.p.traces.iterator();
                  while(tr_.hasNext())
                  {
                     TraceResult tr__ = tr_.next();
                     if(tr__.geolocation != null && tr__.geolocation.equals(tr.geolocation) == false)
                     {
                        addNewConnection(tr__.geolocation, tr.geolocation);
                     }
                  }
               }
   ...
   /**
    * Returns the ip of me exposed to the internet. So the WAN-IP is returned, not the LAN-IP!
    * @return
    */
   private String getMyIP()
   {
      ...
      try
      {
         p = Runtime.getRuntime()
                    .exec(
                       this.p.configFile.getProperty("DIG_PATH") + " +short myip.opendns.com @resolver1.opendns.com");
         BufferedReader input = new BufferedReader(new InputStreamReader(p.getInputStream()));
   ...
   private Coordinate lookupIP(String ip)
   {
      ...
      {
         LookupService ls =
            new LookupService(this.p.configFile.getProperty("GEOIP_CITY_DATABASE"), LookupService.GEOIP_MEMORY_CACHE);
