...
public class Control
{
   ...
   /**
    * What we do here is to make the "loading..." label dynamic by adding dots over time
    */
   private int dots = 0;
   ...
   public void draw()
   {
      @SuppressWarnings("unchecked") Vector<TraceResult> traces_cp = (Vector<TraceResult>)p.traces.clone();
      ...
      for(int a = max; a >= i; a--)
      {
         ...
         els_labels.add(p.controlP5.addTextlabel("lbl" + y, ap.hostname, x, y));
         els_ranges.add(p.controlP5.addRange("" + ap.getAveragePing(), 0, 255, 0, val, x, y + 15, 200, 12));
      ...
      {
         p.fill(p.WHITE);
         p.text("loading " + p.tracer.hostname + getLoadingDots(), x - p.shiftEarthX, yOriginal + 10);
      }
   }
